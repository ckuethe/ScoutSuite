...
class Clusters(Resources):
    ...
    async def _parse_cluster(self, raw_cluster):
        ...
        cluster_dict['application_layer_encryption_enabled'] = raw_cluster.get('databaseEncryption', {}).get('state',
                                                                                                             None) == 'ENCRYPTED'
        cluster_dict['workload_identity_enabled'] = raw_cluster.get('workloadIdentityConfig', {}).get('workloadPool',
                                                                                                      '').endswith(
            '.svc.id.goog')
        cluster_dict['workload_metadata_mode'], cluster_dict[
            'workload_metadata_node_metadata'] = self._get_workload_metadata_config(raw_cluster.get('nodePools', []))
        ...
        cluster_dict['binary_authorization'] = raw_cluster.get('binaryAuthorization', {}).get('evaluationMode',
